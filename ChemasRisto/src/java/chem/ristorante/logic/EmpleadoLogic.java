...
public class EmpleadoLogic extends Logic
{
    ...
    public String VPass(String p_strUsuario)
    {
       
       DatabaseX database = getDatabase();
        String strSql = "select contrasenia from chemas.empleado where usuario = " + "'" + p_strUsuario + "'";
        System.out.println(strSql);
        ResultSet CResult = database.executeQuery(strSql);
        EmpleadoObj CTemp;
        ...
        if(CResult!=null)
        {
            ...
            try 
            {
                while(CResult.next())
                {
                    ...
                    CTemp = new EmpleadoObj(strContrasenia);
                    
                    return CTemp.getContrasenia();
                }
            } 
            catch (SQLException ex) 
            {
